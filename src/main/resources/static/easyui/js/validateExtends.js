	...
	repeat_examination: {
		validator: function (value) {
			...
			$.ajax({
				type: "post",
				async: false,
				url: "ExaminationServlet?method=ExaminationList&t="+new Date().getTime(),
				success: function(data){//在验证函数里加载数据，加载过来后判断输入的值
					var examination = $.parseJSON(data);
		            for(var i=0;i < examination.length;i++){
		            	if(value == examination[i].name){
		            		flag = false;
		            		break;
		            	}
		            }
	...
	repeat_hospital: {
		validator: function (value, param) {
			...
			$.ajax({
				...
				url: "HospitalServlet?method=HospitalList&t="+new Date().getTime(),
				success: function(data){//在验证函数里加载数据，加载过来后判断输入的值
					var hospital = $.parseJSON(data);
		            for(var i=0;i < hospital.length;i++){
		            	if(value == hospital[i].name){
		            		flag = false;
		            		break;
		            	}
		            }
